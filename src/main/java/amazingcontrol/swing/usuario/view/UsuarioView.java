...
public class UsuarioView extends JDialog {
	...
	public UsuarioView(JFrame view) {
		this.view = view;
	...
	private void initPainel() {
		...
		painel.add(form, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.FIRST_LINE_START, 0, new Insets(0, 10, 10, 10), 0, 0));

		painel.add(new JScrollPane(usuariosJTable),
	...
	private void initPopupMenu() {
		...
		//item.addActionListener(new UsuarioAlterarAction(this));
		menu.add(item);
	...
	private void initListeners() {
		novoJButton.addActionListener(new CadastroUsuarioAction(view));
	}
	...
	private void carregarUsuarios() throws Exception {
		...
		for (Usuario usuario: usuarios) {
			...
			model.addRow(new Object[] { nome, ativo});
